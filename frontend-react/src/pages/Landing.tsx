...
const FEATURE_ICONS = [
  <svg key="lang" width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18Z" stroke="currentColor" strokeWidth="1.7" /><path d="M3 12h18M12 3c2.4 2.4 3.6 5.7 3.6 9s-1.2 6.6-3.6 9c-2.4-2.4-3.6-5.7-3.6-9S9.6 5.4 12 3Z" stroke="currentColor" strokeWidth="1.5" /></svg>,
  <svg key="mic2" width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="9" y="2" width="6" height="12" rx="3" stroke="currentColor" strokeWidth="1.7" /><path d="M5 11a7 7 0 0 0 14 0M12 18v3" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" /></svg>,
  <svg key="photo" width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="5" width="18" height="14" rx="2.5" stroke="currentColor" strokeWidth="1.7" /><circle cx="9" cy="11" r="2.2" stroke="currentColor" strokeWidth="1.5" /><path d="M21 16l-5.5-5-4 4-2-1.5L3 18" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>,
  <svg key="track" width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M4 19V5M4 19h16M8 15v-4M12.5 15V7M17 15v-7" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>,
];
...
// like the flow steps, so there's no "first color" reason to start from blue.
...
export default function Landing() {
  ...
  const featureKeys = ["languages", "voice", "photo", "tracking"] as const;
  const stepKeys = ["speak", "translate", "resolve", "reassign", "rate"] as const;

  // Row heights are content-driven (card text length varies), so the 5 nodes are NOT evenly
  // spaced along the line -- a naive "1/4 of the duration apart" delay visibly desyncs from
  ...
  const [nodeGlowDelays, setNodeGlowDelays] = useState<number[]>(() => stepKeys.map(() => 0));
  ...
  useEffect(() => {
    ...
    const recompute = () => {
      ...
      const nodeEls = container.querySelectorAll<HTMLElement>(".flow-node-col");
      const delays = Array.from(nodeEls).map((el) => {
        ...
        return Math.round(((pct - FLOW_GLOW_START_PCT) / (FLOW_GLOW_END_PCT - FLOW_GLOW_START_PCT)) * FLOW_GLOW_DURATION_MS);
      });
      setNodeGlowDelays(delays);
    };
    ...
    // Re-measure on language change too: text length (and so row height) changes with it, and
    // this is a clearer, more explicit trigger than relying solely on ResizeObserver to catch
    // every reflow cause.
  }, [lang]);
          ...
          {stepKeys.map((key, i) => (
            <div
              ...
              style={{
                ...
                "--node-glow-delay": `${nodeGlowDelays[i] ?? 0}ms`,
              } as CSSProperties}
              aria-hidden="true"
            >
